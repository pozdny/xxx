...
class ModelCatalogProduct extends Model {
	...
	public function getProduct($product_id) {
        ...
        $sql = "SELECT ps.size_id as size_id, ps.name as size_p, pv.price FROM ". DB_PREFIX . "product_variants pv LEFT JOIN ". DB_PREFIX . "product_size ps ON (pv.size_id = ps.size_id)  WHERE pv.product_id=".$product_id." ORDER BY pv.price ASC";
        $query = $this->db->query($sql);

        if ($query->num_rows) {
             $variants = $query->rows;
        }
        $sql = "SELECT  tc.complect_id as complect_id, tc.name as complect_name, cs.name as complect_size, cs.size_id as complect_size_id,  pc.price FROM ". DB_PREFIX . "product_complects pc LEFT JOIN ". DB_PREFIX . "complects tc ON (pc.complect_id = tc.complect_id) LEFT JOIN ". DB_PREFIX . "complects_size cs ON (pc.size_id = cs.size_id) WHERE pc.product_id=".$product_id;
        $query = $this->db->query($sql);

        if ($query->num_rows) {
            $complects = $query->rows;
        }
        $sql = "SELECT p2c.category_id as category_id FROM ". DB_PREFIX . "product_to_category p2c WHERE p2c.product_id=".$product_id;
        $query = $this->db->query($sql);
        if ($query->num_rows) {
            $category_parent = $query->rows; //echo '<pre>'; print_r($category_parent); echo '</pre>';
        }
        ...
        if($c_parent){
            $sql = "SELECT cd.name as category_name, cd.category_id as parent_category_id FROM ". DB_PREFIX . "category_description cd WHERE cd.category_id=".(int)$c_parent;
            $query = $this->db->query($sql);
		...
		if (isset($category_parent[0])) {
			$top_parent = $category_parent[0]['category_id'];
		} else {
			$top_parent = 0;
		}
		if($top_parent){
			$sql = "SELECT cd.name as category_name_top_parent, cd.category_id as top_parent_category_id FROM ". DB_PREFIX . "category_description cd WHERE cd.category_id=".(int)$top_parent;
			$query = $this->db->query($sql);
			$category_name_top_parent = $query->row['category_name_top_parent'];
			$top_parent_category_id = $query->row['top_parent_category_id'];
		}
		else{
			$category_name_top_parent = '';
			$top_parent_category_id = '';
		}
        $sql = "SELECT DISTINCT *, pd.name AS name, p.image, m.name AS manufacturer, (SELECT price FROM " . DB_PREFIX . "product_discount pd2 WHERE pd2.product_id = p.product_id AND pd2.customer_group_id = '" . (int)$customer_group_id . "' AND pd2.quantity = '1' AND ((pd2.date_start = '0000-00-00' OR pd2.date_start < '" . $this->NOW . "') AND (pd2.date_end = '0000-00-00' OR pd2.date_end > '" . $this->NOW . "')) ORDER BY pd2.priority ASC, pd2.price ASC LIMIT 1) AS discount, (SELECT price FROM " . DB_PREFIX . "product_special ps WHERE ps.product_id = p.product_id AND ps.customer_group_id = '" . (int)$customer_group_id . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < '" . $this->NOW . "') AND (ps.date_end = '0000-00-00' OR ps.date_end > '" . $this->NOW . "')) ORDER BY ps.priority ASC, ps.price ASC LIMIT 1) AS special, (SELECT points FROM " . DB_PREFIX . "product_reward pr WHERE pr.product_id = p.product_id AND customer_group_id = '" . (int)$customer_group_id . "') AS reward, (SELECT ss.name FROM " . DB_PREFIX . "stock_status ss WHERE ss.stock_status_id = p.stock_status_id AND ss.language_id = '" . (int)$this->config->get('config_language_id') . "') AS stock_status, (SELECT wcd.unit FROM " . DB_PREFIX . "weight_class_description wcd WHERE p.weight_class_id = wcd.weight_class_id AND wcd.language_id = '" . (int)$this->config->get('config_language_id') . "') AS weight_class, (SELECT lcd.unit FROM " . DB_PREFIX . "length_class_description lcd WHERE p.length_class_id = lcd.length_class_id AND lcd.language_id = '" . (int)$this->config->get('config_language_id') . "') AS length_class, (SELECT AVG(rating) AS total FROM " . DB_PREFIX . "review r1 WHERE r1.product_id = p.product_id AND r1.status = '1' GROUP BY r1.product_id) AS rating, (SELECT COUNT(*) AS total FROM " . DB_PREFIX . "review r2 WHERE r2.product_id = p.product_id AND r2.status = '1' GROUP BY r2.product_id) AS reviews, p.sort_order FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) LEFT JOIN " . DB_PREFIX . "manufacturer m ON (p.manufacturer_id = m.manufacturer_id) WHERE p.product_id = '" . (int)$product_id . "' AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p.status = '1' AND p.date_available <= '" . $this->NOW . "' AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'";
		$query = $this->db->query($sql);
		...
		if ($query->num_rows) {
			return array(
				...
				'category_name_top_parent' => $category_name_top_parent,
				'top_parent_category_id'    => $top_parent_category_id
			);
		} else {
			return false;
		}
	}
    public function getProductByCategoryFeatured($product_id){
        if ($this->customer->isLogged()) {
            $customer_group_id = $this->customer->getCustomerGroupId();
        } else {
            $customer_group_id = $this->config->get('config_customer_group_id');
    ...
    public function getProductsByCategoryFeatured($data){ //echo '<pre>'; print_r($this->config); echo '</pre>';
        if ($this->customer->isLogged()) {
            //$customer_group_id = $this->customer->getCustomerGroupId();
        } else {
            //$customer_group_id = $this->config->get('config_customer_group_id');
        ...
        if (!empty($data['filter_category_id'])) {
            $sql .= " INNER JOIN ".DB_PREFIX."product_custom_featured pf ON (p.product_id = pf.product_id) ";
        }
        ...
        $sql .= " WHERE pc.category_id =".$data['filter_category_id'];
	...
	public function getProducts($data = array()) { 
		...
		if (!empty($data['filter_category_id'])) {
			...
			/*if (!empty($data['filter_filter'])) {
				...
				$filter_str = '';
				$filter_arr = explode(',', $data['filter_filter']);
				foreach($filter_arr as $n=>$f){
					$filter_str .= " AND pf.filter_id = ".$f;
				}
				$sql .= $filter_str;
...
//		$this->log->($sql);
		...
		if (!empty($data['filter_filter'])) {
			...
			$sql = "SELECT * FROM " . DB_PREFIX . "product_filter pf WHERE pf.filter_id IN(" . implode(',', $filter_arr) . ")";
			$query2 = $this->db->query($sql);

			$new_arr = array();
			foreach ($query2->rows as $aa) {
				$new_arr[$aa['product_id']][] = $aa['filter_id'];
			}

			foreach ($new_arr as $key_new => $val_new) {
				if (sizeof($val_new) < sizeof($filter_arr) || sizeof($val_new) > sizeof($filter_arr)) {
					unset($new_arr[$key_new]);
					continue;
				}
				foreach ($val_new as $key_new2 => $val_new2) {
					if (in_array($val_new2, $filter_arr)) continue;
					else {
						unset($new_arr[$key_new]);
						continue;
					}
			...
			$filter_arr_new = array();
			foreach ($new_arr as $key_new => $val_new) {
				$filter_arr_new[] = $key_new;
			}

			foreach ($query->rows as $key => $val) {
				if (in_array($val['product_id'], $filter_arr_new)) continue;
				else {
			...
			$arr_rows_new = array();
			foreach ($query->rows as $key_new => $val_new) {
				$arr_rows_new[] = $val_new['product_id'];
			}
			$sql_str = '';
			if (isset($data['sort']) && in_array($data['sort'], $sort_data)) {
				if ($data['sort'] == 'pd.name' || $data['sort'] == 'p.model') {
					$sql_str .= " ORDER BY LCASE(" . $data['sort'] . ")";
				} elseif ($data['sort'] == 'p.price') {
					$sql_str .= " ORDER BY  p.price ";
				} else {
					$sql_str .= " ORDER BY " . $data['sort'];
				}
			} else {
				$sql_str .= " ORDER BY p.sort_order";
			}
			if (isset($data['order']) && ($data['order'] == 'DESC')) {
				$sql_str .= " DESC, LCASE(pd.name) DESC";
			} else {
				$sql_str .= " ASC, LCASE(pd.name) ASC";
			}
			if (isset($data['start']) || isset($data['limit'])) {
				if ($data['start'] < 0) {
					$data['start'] = 0;
				...
				$sql_str .= " LIMIT " . (int)$data['start'] . "," . (int)$data['limit'];
			}
			if(sizeof($arr_rows_new < 1)){
				$arr_rows_new[] = 0;
			}
			$sql = "SELECT SQL_CALC_FOUND_ROWS p.product_id as product_id FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON(p.product_id = pd.product_id) WHERE p.product_id IN(" . implode(',', $arr_rows_new) . ")".$sql_str;
			$query = $this->db->query($sql);
			$num_query = $this->db->query("SELECT FOUND_ROWS() AS `found_rows`");
			$this->FOUND_ROWS = intval($num_query->row['found_rows']);
		}
...
//		$product_data['sql']=$sql;
	...
	public function getColorSelected($product_id) {
		$sql = "SELECT pb.product_bond_product_id as product_id FROM " . DB_PREFIX . "product_bond_color pb  WHERE pb.product_id=".$product_id;
		$query = $this->db->query($sql);
		$color_data = array();
		foreach ($query->rows as $row) { //echo '<pre>'; print_r($row); echo '</pre>';
			$sql = "SELECT * FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) WHERE p.product_id=".$row['product_id'];
			$query = $this->db->query($sql);
			$color_data[] = $this->getProduct($query->row['product_id']);
		}
		return $color_data;
	}
}
