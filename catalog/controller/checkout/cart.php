...
class ControllerCheckoutCart extends Controller {
	...
	public function index() {
    	...
    	if ($this->cart->hasProducts() || $this->cart->hasServices() || !empty($this->session->data['vouchers'])) {
			...
			if($this->cart->hasProducts()){
				...
				$products = $this->cart->getProducts(); //echo '<pre>'; print_r($products); echo '</pre>';
			...
			if($this->cart->hasServices()){
				...
				$this->data['actionServ'] = $this->url->link('checkout/cart/editServ');
	...
	public function add() {
		...
		$json = array();
		if (isset($this->request->post['services_add'])) {
			$this->load->model('catalog/service');
			$services = $this->model_catalog_service->getServices(); //echo '<pre>'; print_r($services); echo '</pre>';
			if (isset($this->request->post['arr_services'])) {
				$arr_services_1 = array();
				$kol = 0;
				$arr_services = $this->request->post['arr_services'];
				foreach($arr_services as $key=>$value){
					if(preg_match("/id_/i", $value['name'])){
						$a = explode('_', $value['name']);
						$id = $a[1];
						$kol = $value['value'];
						if($kol) $arr_services_1[$id] = $kol;
					}

				}
				if(isset($this->session->data['cartServ'])){
					unset($this->session->data['cartServ']);
				}
				foreach($arr_services_1 as $serv_id=>$quantity){
					$this->cart->addServ($serv_id, $quantity);
				}
			}
			$json['success'] = sprintf($this->language->get('text_success_serv'), $this->url->link('checkout/cart'));
			...
			if (isset($this->request->post['economy'])) {
				$economy = $this->request->post['economy'];
				if(!isset($this->session->data['economy'])){
					$this->session->data['economy'] = $economy;
				}
			}
			...
			if ($product_info) {
				...
				//session_destroy();
				if (!$json) {
					$arr_pol_1 = $arr_kom_1 = array();
					if (isset($this->request->post['arr_pol'])) {
						$arr_pol_1 = array();
						$kol = 0;
						$arr_pol = $this->request->post['arr_pol'];
						foreach($arr_pol as $key=>$value){
							if(preg_match("/id_/i", $value['name'])){
								$a = explode('_', $value['name']);
								$id = $a[1];
								$kol = $value['value'];
								if($kol) $arr_pol_1[$id] = $kol;
							}

						}
					}
					//echo '<pre>'; print_r($arr_pol_1); echo '</pre>';
					if (isset($this->request->post['arr_kom'])) {
						$arr_kom_1 = array();
						$kol = 0;
						$arr_kom = $this->request->post['arr_kom'];
						foreach($arr_kom as $key=>$value){
							if(preg_match("/id_/i", $value['name'])){
								$a = explode('_', $value['name']);
								$id = $a[1];
								$kol = $value['value'];
								if($kol) $arr_kom_1[$id] = $kol;
							}

						}
					}
					$this->cart->add($this->request->post['product_id'], $quantity, $option, $arr_pol_1, $arr_kom_1);
					...
					$json['total_quant'] = $this->cart->countProducts() + + $this->cart->countServices() + (isset($this->session->data['vouchers']) ? count($this->session->data['vouchers']) : 0);
    ...
    public function edit() {
        ...
        // Update
        if (!empty($this->request->post['quantity'])) { //echo '<pre>'; print_r($this->request->post['quantity']); echo '</pre>';
	...
	public function editServ() {
		...
		if (!empty($this->request->post['quantity'])) { //echo '<pre>'; print_r($this->request->post['quantity']); echo '</pre>';
			foreach ($this->request->post['quantity'] as $key => $value) {
				$this->cart->updateServ($key, $value);
			}
			$this->response->redirect($this->url->link('checkout/cart'));
		}
	...
	public function removeServ() {
		...
		// Remove
		if (isset($this->request->post['key'])) {
			$this->cart->removeServ($this->request->post['key']);
