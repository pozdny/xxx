...
class ModelCatalogFilter extends Model {
	...
	public function editFilter($filter_group_id, $data) { //echo '<pre>'; print_r($data); echo '</pre>';
		...
		foreach ($data['filter_group_description'] as $language_id => $value) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "filter_group_description SET filter_group_id = '" . (int)$filter_group_id . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($value['name']) . "', alias = '" . $this->db->escape($value['alias']) . "' ");
		}
		...
		if (isset($data['filter'])) {
			foreach ($data['filter'] as $filter) {
				...
				foreach ($filter['filter_description'] as $language_id => $filter_description) {
					if(isset($filter_description['img'])){
						$this->db->query("INSERT INTO " . DB_PREFIX . "filter_description SET filter_id = '" . (int)$filter_id . "', language_id = '" . (int)$language_id . "', filter_group_id = '" . (int)$filter_group_id . "', name = '" . $this->db->escape($filter_description['name']) . "', img = '" . $this->db->escape($filter_description['img']) . "'");
					}
					else{
						$this->db->query("INSERT INTO " . DB_PREFIX . "filter_description SET filter_id = '" . (int)$filter_id . "', language_id = '" . (int)$language_id . "', filter_group_id = '" . (int)$filter_group_id . "', name = '" . $this->db->escape($filter_description['name']) . "'");
	...
	public function getFilterGroupDescriptions($filter_group_id) {
		...
		foreach ($query->rows as $result) {
			$filter_group_data[$result['language_id']] = array(
				'name' => $result['name'],
				'alias' => $result['alias']
			);
		}
