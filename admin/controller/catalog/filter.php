...
class ControllerCatalogFilter extends Controller {
	...
	protected function getForm() {
		...
		if (isset($this->request->get['filter_group_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
      		$filter_group_info = $this->model_catalog_filter->getFilterGroup($this->request->get['filter_group_id']); //echo '<pre>'; print_r($filter_group_info); echo '</pre>';
    	}
		...
		if (isset($this->request->post['filter_group_description'])) {
			...
			$this->data['filter_group_description'] = $this->model_catalog_filter->getFilterGroupDescriptions($this->request->get['filter_group_id']);//echo '<pre>'; print_r($this->data['filter_group_description']);
		} else {
		...
		if (isset($this->request->post['filters'])) {
		    ...
		    foreach($this->data['languages'] as $language){
				foreach($this->data['filters'] as &$filter){//print_r($filter['filter_description'][$language]);
					if (file_exists(DIR_IMAGE . $filter['filter_description'][$language['language_id']]['img']) && $filter['filter_description'][$language['language_id']]['img'] !='') {
						$filter['filter_description'][$language['language_id']]['src'] = $this->model_tool_image->resize($filter['filter_description'][$language['language_id']]['img'], 100, 100);
					}
					else{
						$filter['filter_description'][$language['language_id']]['src'] = $this->model_tool_image->resize('no_image.jpg', 100, 100);
					}
				}
			}
			//echo '<pre>'; print_r($this->data['filters']); echo '</pre>';

		} else {
			$this->data['filters'] = array();
		}
        $this->data['empty_img'] = $this->model_tool_image->resize('no_image.jpg', 100, 100);
