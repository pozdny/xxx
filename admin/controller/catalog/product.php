...
class ControllerCatalogProduct extends Controller {
	...
	public function filter() {
  	...
  	public function insert() {
		$this->document->addScript('view/javascript/sizes.js');
    	$this->language->load('catalog/product');
  	...
  	public function update() { 
    	...
    	if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) { //echo '<pre>';print_r($this->request->post); echo '</pre>';
  	...
  	protected function getForm() {
		...
		$this->data['entry_other_color'] = $this->language->get('entry_other_color');
        ...
        if($product_info !=null){
            if($product_info['product_featured'] == $product_info['product_id']){
                $this->data['featured_checked'] = 'checked="checked"';
            }
            else{
                $this->data['featured_checked'] = '';
            }
        }
        else{
            $this->data['featured_checked'] = '';
        ...
        // Complects
        $this->data['complects'] = $this->model_catalog_category->getComplects();
		// Filters
        ...
        if (isset($this->request->post['main_complects_id'])) {
            $this->data['main_complects_id'] = $this->request->post['main_complects_id'];
        } elseif (isset($product_info)) {
            $this->data['main_complects_id'] = $this->model_catalog_product->getProductComplectId($this->request->get['product_id']);
        } else {
            $this->data['main_complects_id'] = 0;
        }
    ...
    public function getOptions() {
        ...
        if (isset($this->request->post['category_id'])) {
            ...
            $elem_sizes = array();
            if($action == 'size'){
                $elem_sizes = $this->model_catalog_category->getSize($category_id);
				$json['elem_sizes'] =  $elem_sizes;
            }
            elseif($action == 'complects'){
                $elem_sizes = $this->model_catalog_category->getComplectsSize($category_id);
				$json['elem_sizes'] =  $elem_sizes;
            }
			elseif($action == 'color'){
				if (isset($this->request->post['categories'])){
					...
					$this->load->model('catalog/product');
					$results = array();
					$results_main_category = $this->model_catalog_category->getCategory($category_id);

					foreach($categories as $category){
						$category_info = $this->model_catalog_category->getCategory($category);
                        if($category_info['parent_id'] == $category_id){
							$data = array(
					...
					foreach($results as $result){
						foreach($result as $result2){
							if(!in_array($result2['product_id'], $elem_color)){
								if($product_id){
									if($result2['product_id'] != $product_id){
										$elem_color[] = array(
        ...
        if (isset($this->request->get['product_id'])){
            ...
            else $action = 'sizeSelected';
            $product_id = $this->request->get['product_id'];
            if($action == 'sizeSelected'){
                $elem_sizes = $this->model_catalog_category->getSizeSelected($product_id);
				$json['elem_sizes'] =  $elem_sizes;
            }
            elseif($action == 'sizeComplectsSelected'){
                $elem_sizes = $this->model_catalog_category->getSizeComplectsSelected($product_id);
				$json['elem_sizes'] =  $elem_sizes;
            }
			elseif($action = 'colorSelected'){
				$elem_color = $this->model_catalog_category->getColorSelected($product_id);
				$json['elem_color'] = $elem_color;
			}
