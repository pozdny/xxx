...
class ModelCatalogProduct extends Model {
	public function addProduct($data) {//echo '<pre>'; print_r($data); echo '</pre>';
        ...
        if (isset($data['featured'])) {
            $this->db->query("INSERT INTO " . DB_PREFIX . "product_custom_featured SET product_id = '" . (int)$product_id . "', category_id = '" . (int)$data['main_category_id'] . "'");
        }
		if (isset($data['image'])) {
        ...
        if (isset($data['product_size'])) {
            ...
            foreach($data['product_size'] as $value){
                $arr_price[] = $value['price'];
            }
            $price_size = min($arr_price);
            foreach ($data['product_size'] as $key=>$arr_size) {
                $this->db->query("INSERT INTO " . DB_PREFIX . "product_variants SET product_id = '" . (int)$product_id . "', size_id = '" . (int)$arr_size['size_id'] . "', price = '" . $arr_size['price'] . "'");
            }
            $this->db->query("UPDATE " . DB_PREFIX . "product SET price = '" . (float)$price_size . "' WHERE product_id = '" . (int)$product_id . "'");

        }
        if (isset($data['complect_size'])) {

            foreach ($data['complect_size'] as $key=>$arr_size) {
                $this->db->query("INSERT INTO " . DB_PREFIX . "product_complects SET product_id = '" . (int)$product_id . "', complect_id = '" . (int)$arr_size['valueCatId'] . "', size_id = '" . (int)$arr_size['size_id'] . "', price = '" . $arr_size['price'] . "'");
            }
        }
	...
	public function editProduct($product_id, $data) {//echo '<pre>'; print_r($data); echo '</pre>';
		$this->db->query("UPDATE " . DB_PREFIX . "product SET model = '" . $this->db->escape($data['model']) . "', sku = '" . $this->db->escape($data['sku']) . "', upc = '" . $this->db->escape($data['upc']) . "', ean = '" . $this->db->escape($data['ean']) . "', jan = '" . $this->db->escape($data['jan']) . "', isbn = '" . $this->db->escape($data['isbn']) . "', mpn = '" . $this->db->escape($data['mpn']) . "', location = '" . $this->db->escape($data['location']) . "', quantity = '" . (int)$data['quantity'] . "', minimum = '" . (int)$data['minimum'] . "', subtract = '" . (int)$data['subtract'] . "', stock_status_id = '" . (int)$data['stock_status_id'] . "', date_available = '" . $this->db->escape($data['date_available']) . "', manufacturer_id = '" . (int)$data['manufacturer_id'] . "', shipping = '" . (int)$data['shipping'] . "', price = '" . (float)$data['price'] . "', points = '" . (int)$data['points'] . "', weight = '" . (float)$data['weight'] . "', weight_class_id = '" . (int)$data['weight_class_id'] . "', length = '" . (float)$data['length'] . "', width = '" . (float)$data['width'] . "', height = '" . (float)$data['height'] . "', length_class_id = '" . (int)$data['length_class_id'] . "', status = '" . (int)$data['status'] . "', tax_class_id = '" . $this->db->escape($data['tax_class_id']) . "', sort_order = '" . (int)$data['sort_order'] . "', date_modified = NOW() WHERE product_id = '" . (int)$product_id . "'");
        if (isset($data['featured'])) {
            $this->db->query("DELETE FROM " . DB_PREFIX . "product_custom_featured WHERE product_id = '" . (int)$product_id . "'");
            $this->db->query("INSERT INTO " . DB_PREFIX . "product_custom_featured SET product_id = '" . (int)$product_id . "', category_id = '" . (int)$data['main_category_id'] . "'");
        }
        else{
            $this->db->query("DELETE FROM " . DB_PREFIX . "product_custom_featured WHERE product_id = '" . (int)$product_id . "'");
        ...
        $this->db->query("DELETE FROM " . DB_PREFIX . "product_complects WHERE product_id = '" . (int)$product_id . "'");
        if (isset($data['complect_size'])) {
		...
		$this->db->query("DELETE FROM " . DB_PREFIX . "product_bond_color WHERE product_id = '" . (int)$product_id . "'");
		if (isset($data['colors'])) {

			foreach ($data['colors'] as $key=>$val_color) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "product_bond_color SET product_id = '" . (int)$product_id . "', product_bond_product_id = '" . (int)$val_color . "'");
			}
		}
	...
	public function getProduct($product_id) {
		$query = $this->db->query("SELECT DISTINCT *, (SELECT keyword FROM " . DB_PREFIX . "url_alias WHERE query = 'product_id=" . (int)$product_id . "') AS keyword, (SELECT product_id FROM " . DB_PREFIX . "product_custom_featured WHERE product_id = " . (int)$product_id . ") AS product_featured FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) WHERE p.product_id = '" . (int)$product_id . "' AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "'");
    ...
    public function getProductComplectId($product_id) {
        $query = $this->db->query("SELECT complect_id FROM " . DB_PREFIX . "product_to_category WHERE product_id = '" . (int)$product_id . "'");

        return ($query->num_rows ? (int)$query->row['complect_id'] : 0);
    }
